---
# Copy this file to a new name (e.g. why-brain-fog-persists.md). Files starting
# with "_" are ignored by the site, so this template never publishes itself.
#
# Required
title: "Your Post Title"
date: 2026-09-16
category: article            # "article" or "research-review"
excerpt: "One or two sentences shown on the listing card and used as the meta description (aim for 140–160 characters)."

# Optional
# slug: custom-url-slug      # defaults to the filename
# updatedAt: 2026-10-01
# author: "Dr. Chris Slininger"   # defaults to "mTBI Research Team"
# authorType: Person         # Person or Organization (auto-detects "Dr." as Person)
# image: /images/blog/your-image.jpg   # 1200x630 recommended; used for social sharing
tags: [mTBI, craniocervical junction, research]
# draft: true                # keeps the post out of the site until removed
---

Open with the answer or the point in the first paragraph — readers and AI search
engines both reward it.

## Use question-style headings where it fits

Body text in Markdown. **Bold**, _italics_, [links](https://example.com), lists:

- Point one
- Point two

> Pull quotes render as styled blockquotes.

## For a Research Review

Summarize the study in plain language, then what it means for this research.

### Sources

1. Author A, Author B. *Title of the paper*. Journal. Year. [DOI or link](https://doi.org/...)
